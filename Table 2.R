...
for(i in 2:5)
{
  for(j in 1:5)
  {
    for(c in 1:10)
    {
      ...
      load(paste("Scenario",order.num,".RData",sep=""))
      ## all simulation datasets in "Table 2 Simulation Data Part 1.zip" and "Table 2 Simulation Data Part 2.zip" should be stored into the work space
      if(j==3)
      {
...
write.csv(x=data.results,file="Table2.csv")